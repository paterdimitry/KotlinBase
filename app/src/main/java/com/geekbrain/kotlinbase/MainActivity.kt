...
class MainActivity : AppCompatActivity() {
    ...
    private fun printCatName(cats: List<Cat>) {
        for(cat in cats) {
            println("Имя кота " + cat.name)
        }
    }

    private fun colorCat(cats: List<Cat>) {
        val cat = cats[0].copy(color = "black")
        println("Кот по имени ${cat.name} возраста ${cat.age} лет и его цвет - ${cat.color} \n")
    }
}
